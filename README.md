...
**Purpose:** Assess account protection level. MFA was disabled, increasing compromise risk.

### 3) Phishing Email Discovery
...
**Purpose:** Identify spear-phishing messages impersonating a luxury brand.

### 4) Confirm Phishing Link Click
...
**Purpose:** Confirm the user interacted with the malicious infrastructure.

### 5) Infrastructure Pivoting (Domain → IP → Reuse)
...
**Purpose:** Identify the hosting IP behind the phishing site.
...
**Purpose:** Detect reuse of hosting infrastructure across multiple malicious domains.

### 6) Inbound Reconnaissance Analysis
...
**Purpose:** Identify pre-attack reconnaissance behavior.
...
**Purpose:** Reveal attacker intent by analyzing URL patterns rather than raw traffic.

**Finding:** Recon focused on the victim’s location, consistent with OSINT-driven targeting.

### 7) Attacker Attribution via PassiveDNS
...
**Purpose:** Correlate malicious login activity with phishing infrastructure.

### 8) Email Exfiltration Detection (Critical Phase)
...
**Purpose:** Confirm available fields (`reply_to`, `links`, `attachments`) before hunting exfiltration.
...
**Purpose:** Establish baseline and confirm external email volume suitable for exfiltration analysis.
...
**Purpose:** Forwarded messages preserve mailbox ownership via `reply_to`, not sender. This pivot reveals auto-forwarded exfiltration.
...
**Purpose:** Identify identity-theft-grade data exfiltration. Documents were shared via cloud links, not attachments.
...
**Purpose:** Confirm theft of financial documents enabling fraud and tax abuse.
